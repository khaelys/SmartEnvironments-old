...
public class TensorFlowInferenceInterface {
  ...
  private static final String ASSET_FILE_PREFIX = "/storage/emulated/0/Android/data/com.unime.tensorflowproject/files/Download/";

  /*
   * Load a TensorFlow model from the AssetManager or from disk if it is not an asset file.
   *
   * @param assetManager The AssetManager to use to load the model file.A
  ...
  public void run(String[] outputNames, boolean enableStats) {
    ...
    try {
      if (enableStats) {
        Session.Run r = runner.setOptions(org.tensorflow.contrib.android.RunStats.runOptions()).runAndFetchMetadata();
        fetchTensors = r.outputs;

        if (runStats == null) {
          runStats = new org.tensorflow.contrib.android.RunStats();
        }
